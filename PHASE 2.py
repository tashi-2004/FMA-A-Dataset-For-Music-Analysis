...
def train_music_recommendation_model(input_uri, output_uri):
    ...
    try:
        ...
        # Save the split data with overwrite option
        trainingData.write.format("parquet").mode("overwrite").save("training_data_path")
        testData.write.format("parquet").mode("overwrite").save("test_data_path")
...
if __name__ == "__main__":
    ...
    # Prompt the user to input a track ID or 'exit' to end the program
    while True:
        user_input = input("Enter a track ID (0 to {}), 'exit' to end, or 'add' to input another track ID: ".format(num_tracks - 1))
        ...
        try:
            ...
            print("Invalid input. Please enter a valid integer, 'exit' to end, or 'add' to input another track ID.")
